...
MAVLINK_GIT_BRANCHNAME=aviant/main
...
# Determine branch prefix based on event type
if [ "$GITHUB_EVENT_NAME" = "pull_request" ]; then
    BRANCH_PREFIX="auto-update-pr"
elif [ "$GITHUB_REF" = "refs/heads/aviant/main" ]; then
    BRANCH_PREFIX="auto-update-merged"
else
    echo "ERROR: Unexpected context - not PR or main branch (GITHUB_REF=$GITHUB_REF)"
    exit 1
fi

CLIBRARY_GIT_BRANCHNAME="${BRANCH_PREFIX}-${MAVLINK_GITHASH:0:8}"
...
cd $CLIBRARY_PATH
git checkout -b $CLIBRARY_GIT_BRANCHNAME
git add --all :/ || exit 1
COMMIT_MESSAGE="autogenerated headers for rev https://github.com/aviant-tech/mavlink/tree/"$MAVLINK_GITHASH
GIT_COMMITTER_NAME="Aviant Bot" \
GIT_COMMITTER_EMAIL="bot@aviant.no" \
git commit -m "$COMMIT_MESSAGE" --author "Aviant Bot <bot@aviant.no>" || exit 0  # for the case when nothing is there to commit

# push to c_library repository
git push -u $CLIBRARY_GIT_REMOTENAME $CLIBRARY_GIT_BRANCHNAME || exit 1
echo -e "\0033[34mBranch $CLIBRARY_GIT_BRANCHNAME pushed successfully\0033[0m"
echo -e "\0033[34mCreate PR at: https://github.com/aviant-tech/c_library_v2/compare/$CLIBRARY_GIT_BRANCHNAME\0033[0m"
