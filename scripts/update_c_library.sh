...
if [ "${PUSH_TO_MAIN:-}" = "true" ]; then
    CLIBRARY_GIT_BRANCHNAME=""
else
    CLIBRARY_GIT_BRANCHNAME="auto-update-${MAVLINK_GITHASH:0:8}"
fi
...
cd $CLIBRARY_PATH
if [ "${PUSH_TO_MAIN:-}" = "true" ]; then
    CLIBRARY_GIT_BRANCHNAME=$(git rev-parse --abbrev-ref HEAD)
else
...
if [ "${PUSH_TO_MAIN:-}" = "true" ]; then
    echo -e "\0033[34mPushed to $CLIBRARY_GIT_BRANCHNAME in c_library_v2\0033[0m"
else
